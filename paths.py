def Check_Corners(height: int, width: int, entry_row: int, entry_column: int, exit_row: int, exit_column: int) -> None:
    if entry_row < 0 or entry_row >= height:
        raise ValueError("entry row is outside the maze borders!")

    if entry_column < 0 or entry_column >= width:
        raise ValueError("entry column is outside the maze borders!")

    if exit_row < 0 or exit_row >= height:
        raise ValueError("exit row is outside the maze borders!")

    if exit_column < 0 or exit_column >= width:
        raise ValueError("exit column is outside the maze borders!")
